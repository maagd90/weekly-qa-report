...
function projectRows(dataset: Dataset): Array<{ project: string; rows: number }> {
  ...
  const add = (project?: string, rows = 1) => { const key = canonicalProjectKey(project || 'UNKNOWN') || 'UNKNOWN'; counts.set(key, (counts.get(key) || 0) + rows); };
...
router.post('/report/pdf', async (req: Request, res: Response) => { const { startDate, endDate, reportType, kpiStyle, project, branding } = req.body as { startDate?: string; endDate?: string; reportType?: string; kpiStyle?: string; project?: string; branding?: ReportBrandingPayload }; const clean = cleanProject(project); const connections = resolveConnections(req); log(req, 'POST /report/pdf:start', { startDate, endDate, reportType, kpiStyle, project: clean, hasLogo: Boolean(branding?.logoUrl), connections: connectionSummary(connections) }); if (!startDate || !endDate) return res.status(400).json({ error: 'startDate and endDate are required', requestId: requestId(req) }); const type = ['full', 'executive', 'testers', 'cycles'].includes(reportType || '') ? reportType! : 'executive'; const kpi = ['editorial', 'framed', 'minimal'].includes(kpiStyle || '') ? kpiStyle! : 'editorial'; const cached = await ensureDataset(req, connections, undefined, 'cached'); if (!cached) return res.status(404).json({ error: 'No dashboard data. Search or sync data before downloading PDF. Dataset contains: no projects / 0 rows.', requestId: requestId(req) }); const payload = refilterDashboard(cached.dataset, { startDate, endDate, project: clean }); if (!hasMetrics(payload)) return res.status(404).json({ error: noMetricsMessage(clean, startDate, endDate, cached.dataset), requestId: requestId(req), projects: projectRows(cached.dataset) }); try { const pdfBuffer = await generateReportPdf(startDate, endDate, type, kpi, clean, branding); const suffix = clean ? `-${clean}` : ''; const filename = `qa-report${suffix}-${startDate}-to-${endDate}.pdf`; res.setHeader('Content-Type', 'application/pdf'); res.setHeader('Content-Disposition', `attachment; filename="${filename}"`); res.send(pdfBuffer); } catch (err) { logError(req, 'POST /report/pdf:failed', err, { project: clean }); res.status(500).json({ error: `PDF generation failed: ${(err as Error).message}`, requestId: requestId(req) }); } });
