...
async function refreshGeneratedOutputs(req: Request, connections: UserConnections, apiScope?: ApiFetchScope, dashboardFilter: Partial<FilterParams> = {}) {
  const options = { apiScope: cleanApiScope(apiScope) };
  ...
  if (totalRows(dataset) === 0) {
    ...
    log(req, 'generated outputs cleared; no source data remains', { rowCounts: counts, removed, apiScope: options.apiScope });
  ...
  log(req, 'generated outputs refreshed', { rowCounts: counts, warnings: dataset.meta.warnings, projects: dataset.projects, apiScope: options.apiScope });
...
async function ensureDataset(req: Request, connections: UserConnections, apiScope?: ApiFetchScope): Promise<{ dataset: Dataset; fingerprint: string } | null> {
  const options = { apiScope: cleanApiScope(apiScope) };
  const fingerprint = computeFingerprint(INPUT_DIR, CONFIG_DIR, connections, options);
  const cached = loadFingerprint(OUTPUT_DIR);
  let dataset = loadRawDataset(OUTPUT_DIR);
  log(req, 'ensureDataset:start', { inputDir: INPUT_DIR, configDir: CONFIG_DIR, outputDir: OUTPUT_DIR, fingerprint, cachedFingerprint: cached, apiScope: options.apiScope });
...
router.get('/dashboard', async (req: Request, res: Response) => {
  ...
  log(req, 'GET /dashboard:start', { filter, apiScope, connections: connectionSummary(connections) });
  const cached = await ensureDataset(req, connections, apiScope);
  if (cached) return res.json(refilterDashboard(cached.dataset, filter));
  const file = path.join(OUTPUT_DIR, 'dashboard-data.json');
  if (!fs.existsSync(file)) return res.status(404).json({ error: 'No dashboard generated yet. Select dates and click Search APIs, or sync/import data.', requestId: requestId(req) });
...
router.post('/dashboard/search', async (req: Request, res: Response) => {
  ...
  const connections = resolveConnections(req);
  const filter: FilterParams = { startDate, endDate, search, result: result || 'all', project };
  const apiScope = apiScopeFromFilter(filter);
  log(req, 'POST /dashboard/search:start', { filter, apiScope, connections: connectionSummary(connections) });
  if (!startDate || !endDate) return res.status(400).json({ ok: false, error: 'startDate and endDate are required', requestId: requestId(req) });
  try {
    const sync = await refreshGeneratedOutputs(req, connections, apiScope, filter);
    return res.json({ ok: true, dashboard: sync.dashboard, rowCounts: sync.rowCounts, warnings: sync.warnings, projects: sync.projects, requestId: requestId(req) });
  } catch (err) { logError(req, 'POST /dashboard/search:failed', err); return res.status(500).json({ ok: false, error: (err as Error).message, requestId: requestId(req) }); }
});
...
router.get('/cycles/folders', async (req: Request, res: Response) => {
  ...
  if (conn) { try { const cfg = qmetryConfigFromConnection(conn); const folders = await fetchProjectFolders(cfg); const cycles = await fetchProjectCycles(cfg); log(req, 'GET /cycles/folders:qmetry result', { connection: conn.name, folders: folders.length, cycles: cycles.length }); return res.json({ source: 'qmetry-live', connection: conn.name, connectionId: conn.id, folders, cycles }); } catch (err) { logError(req, `GET /cycles/folders:qmetry failed for ${conn.name}`, err); } }
...
router.get('/cycles/by-folder', async (req: Request, res: Response) => {
  ...
  if (conn) { try { const result = await fetchFolderCycleHealth(qmetryConfigFromConnection(conn), folderId === 'all' ? undefined : folderId); log(req, 'GET /cycles/by-folder:qmetry result', { connection: conn.name, folderId, cycles: result.cycles.length, error: result.error }); return res.json({ source: 'qmetry-live', connection: conn.name, connectionId: conn.id, folderId, cycles: result.cycles, warnings: result.error ? [result.error] : [] }); } catch (err) { logError(req, `GET /cycles/by-folder:qmetry failed for ${conn.name}`, err); } }
  const cached = await ensureDataset(req, connections);
...
router.post('/report/pdf', async (req: Request, res: Response) => {
  ...
  const apiScope = cleanApiScope({ startDate, endDate, project });
  const cached = await ensureDataset(req, connections, apiScope);
  if (!cached) return res.status(404).json({ error: 'No dashboard data. Select dates and click Search APIs first.', requestId: requestId(req) });
...
router.post('/integrations/test', async (req: Request, res: Response) => { const connections = resolveConnections(req); log(req, 'POST /integrations/test:start', { connections: connectionSummary(connections) }); try { const dataset = await buildDataset(INPUT_DIR, CONFIG_DIR, connections); log(req, 'POST /integrations/test:done', { executions: dataset.executions.length, issues: dataset.issues.length, uat: dataset.uat.length, warnings: dataset.meta.warnings }); res.json({ ok: true, executions: dataset.executions.length, issues: dataset.issues.length, uat: dataset.uat.length, warnings: dataset.meta.warnings }); } catch (err) { logError(req, 'POST /integrations/test:failed', err); res.status(500).json({ ok: false, error: (err as Error).message, requestId: requestId(req) }); } });
...
router.post('/input/sync', async (req: Request, res: Response) => {
  const connections = resolveConnections(req);
  const { startDate, endDate, search, result, project } = req.body as Partial<FilterParams>;
  const filter: FilterParams = { startDate, endDate, search, result: result || 'all', project };
  const apiScope = apiScopeFromFilter(filter);
  log(req, 'POST /input/sync:start', { filter, apiScope, connections: connectionSummary(connections) });
  try { const sync = await refreshGeneratedOutputs(req, connections, apiScope, filter); return res.json({ ok: true, ...sync, requestId: requestId(req) }); } catch (err) { logError(req, 'POST /input/sync:failed', err); return res.status(500).json({ ok: false, error: (err as Error).message, requestId: requestId(req) }); }
});
