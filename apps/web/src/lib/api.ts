...
function canonicalizeJql(jql?: string): string { return (jql || '').trim(); }
function normalizeWorkspaceName(c: { workspaceName?: string; name?: string }): string { return (c.workspaceName || c.name || '').trim(); }
function normalizeJiraConnection(c: JiraConnectionInput): JiraConnectionInput { const projectKeys = uniqueCanonicalProjects(c.projectKeys || []); const jql = canonicalizeJql(c.jql) || projectJql(projectKeys); const workspaceName = normalizeWorkspaceName(c); return { ...c, name: c.name || workspaceName, workspaceName, deploymentType: c.deploymentType || 'on-prem', enabled: c.enabled !== false, syncIssues: c.syncIssues !== false, projectKeys, jql }; }
function normalizeQmetryConnection(c: QmetryConnectionInput): QmetryConnectionInput { const workspaceName = normalizeWorkspaceName(c); return { ...c, name: c.name || workspaceName, workspaceName, enabled: c.enabled !== false, syncExecutions: c.syncExecutions !== false, projectKey: canonicalProjectOrUndefined(c.projectKey) || '', cycleIds: [] }; }
function normalizeFilter(filter?: Partial<FilterParams>): Partial<FilterParams> | undefined { if (!filter) return undefined; return { ...filter, project: filter.project === 'all' ? undefined : filter.project }; }
function normalizeGenerate(params: GenerateParams): GenerateParams { return { ...params, project: params.project === 'all' ? undefined : params.project }; }
...
function modelForProvider(provider: LlmProvider, raw?: string): string { const clean = (raw || '').trim(); if (provider === 'openai-compatible') return clean; return clean && LLM_MODELS[provider].includes(clean) ? clean : LLM_MODELS[provider][0]; }
...
export function getUserLlmSelection(): LlmSelectionInput { const stored = readStoredObject<Partial<LlmSelectionInput>>(LLM_SELECTION_STORAGE, {}); const provider = normalizeProvider(stored.provider); const model = modelForProvider(provider, stored.model); const apiKey = getUserLlmKey(provider); return { provider, model, baseUrl: stored.baseUrl || undefined, apiKey: apiKey || undefined }; }
export function setUserLlmSelection(selection: LlmSelectionInput): void { const provider = normalizeProvider(selection.provider); const model = modelForProvider(provider, selection.model); writeStoredObject(LLM_SELECTION_STORAGE, { provider, model, baseUrl: selection.baseUrl || '' }); setUserLlmKey(provider, selection.apiKey || ''); }
...
export const batchApi = {
  ...
  getReport: (project?: string) => api.get('/report', { params: { project: project === 'all' ? undefined : project } }).then((r) => r.data).catch((err) => { if (axios.isAxiosError(err) && err.response?.status === 404) return null; throw err; }),
  upload: (file: File, project?: string) => { const form = new FormData(); form.append('file', file); return api.post('/upload', form, { params: { project: project === 'all' ? undefined : project }, headers: { 'Content-Type': 'multipart/form-data' } }).then((r) => r.data); },
  ...
  listInputFiles: (project?: string) => api.get('/input/files', { params: { project: project === 'all' ? undefined : project } }).then((r) => r.data as { name: string; size: number; modifiedAt: string; project?: string }[]),
  deleteInputFile: (filename: string, project?: string) => api.delete(`/input/${encodeURIComponent(filename)}`, { params: { project: project === 'all' ? undefined : project } }).then((r) => r.data),
  ...
  getCycleFolders: (connectionId?: unknown, project?: string) => { const id = typeof connectionId === 'string' ? connectionId : undefined; return api.get('/cycles/folders', { params: { connectionId: id, project: project === 'all' ? undefined : project }, timeout: 45_000 }).then((r) => r.data as CycleFoldersResult); },
  ...
  downloadReportPdf: async ({ startDate, endDate, reportType, kpiStyle, project, branding }: { startDate: string; endDate: string; reportType: ReportType; kpiStyle: string; project?: string; branding?: ReportBranding }) => {
    const cleanProject = project === 'all' ? undefined : project;
    try { const selectedBranding = branding || getReportBranding(); const response = await api.post('/report/pdf', { startDate, endDate, reportType, kpiStyle, project: cleanProject, branding: selectedBranding }, { responseType: 'blob', timeout: 150_000 }); const blob = response.data as Blob; if (blob.type === 'application/json') throw new Error(await blobErrorMessage(blob, 'PDF export failed')); const url = URL.createObjectURL(blob); const suffix = cleanProject ? `-${cleanProject}` : ''; const link = document.createElement('a'); link.href = url; link.download = `qa-report${suffix}-${startDate}-to-${endDate}.pdf`; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url); }
