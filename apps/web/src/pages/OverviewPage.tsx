      ...
      <QaSection>
        <div className="flex flex-wrap gap-6">
          <div className="max-w-[430px] flex-1 min-w-[240px]">
            ...
            <p className="m-0 mb-3.5 text-[11.5px] text-qa-muted-light">Issue Type column · JIRA ({fmt(storyBug.story + storyBug.bug)} issues in scope).</p>
            {storyBug.story + storyBug.bug === 0 ? <div className="py-5 text-[12.5px] text-qa-muted-light">No work items match the current filters.</div> : (
              <div className="flex flex-col gap-3.5">
                {[
                  ...
                  <div key={r.label}>
                    <div className="flex justify-between items-baseline mb-1"><span className="flex items-center gap-2 text-[13px] font-semibold"><span className="w-[11px] h-[11px]" style={{ background: r.color }} />{r.label}</span><span className="font-mono-qa text-xs text-qa-muted">{fmt(r.count)} cases · {r.done} {r.closedLabel} · {r.open} open · {Math.round((r.count / sbTot) * 100)}%</span></div>
          ...
          <div className="flex-1 min-w-[240px] border-l border-[#efece4] pl-6">
            ...
            <div className="flex flex-col gap-4">
              {[
                ...
                <div key={b.label}><div className="flex justify-between items-baseline mb-1"><span className="text-[12.5px] font-semibold" style={{ color: b.doneColor }}>{b.label}</span><span className="font-mono-qa text-[11px] text-qa-muted">{b.done} done · {b.open} open</span></div><div className="flex h-3.5 bg-qa-track"><div style={{ width: `${(b.done / b.total) * 100}%`, background: b.doneColor, height: '100%' }} /><div style={{ width: `${(b.open / b.total) * 100}%`, background: b.openColor, height: '100%' }} /></div></div>
              ))}
            </div>
            <div className="flex gap-4 mt-3.5 pt-3 border-t border-[#efece4] text-[10.5px] text-qa-muted"><span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 bg-[#15605E]" />Done / Closed</span><span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 bg-[#cbb89a]" />Open</span></div>
            ...
            {defectBacklog.openTotal > 0 && <p className="text-[11px] text-qa-muted mt-2 m-0">{defectBacklog.openTotal} open bugs · {defectBacklog.byPriority.find((p) => p.priority === 'Highest')?.open ?? 0} at Highest priority</p>}
          </div>
        </div>
      </QaSection>
    </>
