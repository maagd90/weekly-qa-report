...
export function AiReportPage({ dashboard, kpiStyle, project }: AiReportPageProps) {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      <div className="px-8 py-4 border-b border-qa-border print:hidden">
        <div className="flex flex-wrap items-end gap-3">
          <div><label className="font-mono-qa text-[10px] uppercase tracking-wider text-qa-muted-light block mb-1">Start</label><input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="border border-qa-ink px-2 py-1 text-sm bg-white" /></div>
          <div><label className="font-mono-qa text-[10px] uppercase tracking-wider text-qa-muted-light block mb-1">End</label><input type="date" value={endDate} max={today} onChange={(e) => setEndDate(e.target.value)} className="border border-qa-ink px-2 py-1 text-sm bg-white" /></div>
          <div><label className="font-mono-qa text-[10px] uppercase tracking-wider text-qa-muted-light block mb-1">Report type</label><select value={reportType} onChange={(e) => setReportType(e.target.value as ReportType)} className="border border-qa-ink px-2 py-1 text-sm bg-white">{REPORT_TYPES.map((t) => <option key={t.value} value={t.value}>{t.label} — {t.desc}</option>)}</select></div>
          <div><label className="font-mono-qa text-[10px] uppercase tracking-wider text-qa-muted-light block mb-1">Project</label><select value={reportProject} onChange={(e) => setReportProject(e.target.value)} className="border border-qa-ink px-2 py-1 text-sm bg-white">{projectOptions.map((p) => <option key={p} value={p}>{p === 'all' ? 'All projects' : projectDisplayName(p)}</option>)}</select></div>
          ...
          <button type="button" onClick={() => generateMutation.mutate()} disabled={generating} className={clsx('px-4 py-2 text-xs font-mono-qa uppercase tracking-wider text-white border-0', generating ? 'opacity-60 cursor-wait' : 'cursor-pointer')} style={{ background: QA.accent }}>{generating ? 'Generating...' : 'Generate AI report'}</button>
          <button type="button" onClick={downloadPdf} disabled={downloading || !chartData} className="px-4 py-2 text-xs font-mono-qa uppercase tracking-wider border border-qa-ink bg-white disabled:opacity-50 inline-flex items-center gap-2"><Download size={14} />{downloading ? 'Exporting...' : 'Download PDF'}</button>
        </div>
      ...
      <main className="flex-1 overflow-auto p-8 bg-[#f5f3ed]">
        <div className="max-w-qa mx-auto bg-white border border-qa-border shadow-sm min-h-[600px] print:border-0 print:shadow-none">
          ...
          {hasReport ? (
            ...
            <div className="p-12 text-center text-qa-muted">
