...
export function AiReportPage({ dashboard, kpiStyle, onGenerated }: AiReportPageProps) {
  ...
  const generateMutation = useMutation({
    mutationFn: () => batchApi.generate({ startDate, endDate, reportType, llm: getUserLlmSelection() }),
    onSuccess: (result) => {
          ...
          <aside className="lg:sticky lg:top-[18px]">
            <div className="bg-qa-ink text-[#F5F3ED] p-[22px]">
              ...
              <div className="flex flex-col gap-1.5 mb-4">
                {REPORT_TYPES.map((t) => (
                  <button
                    ...
                    className={clsx('flex items-baseline gap-2 px-2.5 py-2 border cursor-pointer text-left text-[12.5px] font-semibold', reportType === t.value ? 'border-qa-accent bg-[#2a2825]' : 'border-[#3a3833] bg-transparent')}
              ...
              <div className="flex flex-wrap gap-1 mb-4">
                {datePresets.map((d) => (
                  <button key={d.label} type="button" onClick={() => { setEndDate(today); setStartDate(new Date(Date.now() - d.days * 86400000).toISOString().slice(0, 10)); }} className="font-mono-qa text-[10px] px-2 py-1 border border-[#44423d] bg-[#2a2825] text-[#F5F3ED] cursor-pointer">{d.label}</button>
                ))}
              </div>
              ...
              <div className="font-mono-qa text-[11.5px] text-[#F5F3ED] bg-[#2a2825] px-2.5 py-2 mb-4">
                {savedProviderLabel} · {savedLlm.model || 'default model'}
              </div>
