...
import { batchApi, getJiraConnections, setJiraConnections, getQmetryConnections, setQmetryConnections, newConnectionId, getUserLlmSelection, setUserLlmSelection, getUserLlmKey, getReportBranding, setReportBranding, LLM_MODELS, LLM_PROVIDER_LABELS } from '../lib/api';
...
interface TestResult { ok: boolean; count?: number; executions?: number; issues?: number; uat?: number; error?: string }
...
function JiraConnectionCard({ conn, onChange, onRemove, projectOptions }: { conn: JiraConnectionInput; onChange: (next: JiraConnectionInput) => void; onRemove: () => void; projectOptions: string[] }) {
  ...
  const testMutation = useMutation({ mutationFn: () => batchApi.testConnection('jira', { ...conn, enabled, syncIssues }), onSuccess: (r) => setTestResult(r) });
  return (
...
function QmetryConnectionCard({ conn, onChange, onRemove }: { conn: QmetryConnectionInput; onChange: (next: QmetryConnectionInput) => void; onRemove: () => void }) {
  ...
  return (
    <div className="border border-qa-border bg-[#faf8f2] p-3.5 mb-3">
      ...
      <div className="flex items-center gap-2 mt-2.5"><button type="button" onClick={() => setShowSecret((v) => !v)} className="font-mono-qa text-[10px] px-3 py-1.5 border border-qa-border bg-white cursor-pointer">{showSecret ? 'Hide values' : 'Show values'}</button><button type="button" onClick={() => testMutation.mutate()} disabled={testMutation.isPending || !enabled} className="font-mono-qa text-[10px] px-3 py-1.5 border border-qa-border bg-white cursor-pointer disabled:opacity-50">{testMutation.isPending ? 'Testing...' : 'Test cycle search'}</button><button type="button" onClick={onRemove} className="font-mono-qa text-[10px] text-[#a13d2c] underline bg-transparent border-none cursor-pointer">Remove</button>{testResult && <span className={`font-mono-qa text-[10.5px] ${testResult.ok ? 'text-[#2f6a48]' : 'text-[#a13d2c]'}`}>{testResult.ok ? `OK (${testResult.count ?? 0} test cycles found)` : testResult.error}</span>}</div>
    </div>
  );
}
...
export function SettingsPage() {
  ...
  function saveAll() { setUserLlmSelection(currentLlmSelection()); setReportBranding(reportBranding); setJiraConnections(jiraConnections); setQmetryConnections(qmetryConnections.map((c) => ({ ...c, cycleIds: [] }))); setSavedMsg('Saved. Click Sync imported data to refresh uploaded files plus JIRA and QMetry APIs.'); queryClient.invalidateQueries({ queryKey: ['integrations'] }); queryClient.invalidateQueries({ queryKey: ['cycles-folders'] }); queryClient.invalidateQueries({ queryKey: ['dashboard'] }); queryClient.invalidateQueries({ queryKey: ['dashboard-init'] }); setTimeout(() => setSavedMsg(null), 3500); }
