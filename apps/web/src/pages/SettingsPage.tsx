...
QMETRY_BASIC_AUTH=Basic xxxxx`}</pre>
        ...
        <QaSection title="Claude API Key">
          <p className="text-[13px] text-qa-muted m-0 mb-3">
            Set in .env for AI Report generation. Proxy is opt-in via{' '}
            <code className="font-mono-qa text-xs">config/report.json</code> (
            <code className="font-mono-qa text-xs">proxy.enabled</code>).
          </p>
          ...
          {anthropicResult && (
            <div className={`mt-3 p-3 text-[13px] border ${anthropicResult.ok ? 'border-[#cfe0d4] bg-[#eef4ef] text-[#2f6a48]' : 'border-[#ecccc2] bg-[#f8ece8] text-[#a13d2c]'}`}>
              {anthropicResult.ok
                ? `Connected — model ${anthropicResult.model} (${anthropicResult.route ?? (anthropicResult.proxyUsed ? 'proxy' : 'direct')})${anthropicResult.proxyUrl ? ` · ${anthropicResult.proxyUrl}` : ''}${anthropicResult.elapsedMs != null ? ` · ${anthropicResult.elapsedMs}ms` : ''}`
