...
function SyncBox({ label, note, checked, disabled, onChange }: { label: string; note?: string; checked: boolean; disabled?: boolean; onChange: (checked: boolean) => void }) {
  return (
    <label className={`inline-flex items-start gap-2 text-[12.5px] text-qa-ink ${disabled ? 'opacity-50' : ''}`}>
      ...
      <span><span className="font-semibold">{label}</span>{note && <span className="block text-[11px] text-qa-muted-light mt-0.5">{note}</span>}</span>
    </label>
  );
}
  ...
  return (
    <div className="border border-qa-border bg-[#faf8f2] p-3.5 mb-3">
      <div className="flex flex-wrap gap-5 mb-4 p-3 border border-qa-border bg-white">
        ...
        <SyncBox label="Sync JIRA tickets / bugs & stories" note="Used only by Overview Search and Settings Sync JIRA/QMetry." checked={syncIssues} disabled={!enabled} onChange={(checked) => onChange({ ...conn, syncIssues: checked })} />
      </div>
...
export function SettingsPage() {
  ...
  const syncLiveMutation = useMutation({
    mutationFn: () => batchApi.syncLiveData(),
    onSuccess: async () => {
      await Promise.all([refetch(), queryClient.invalidateQueries({ queryKey: ['dashboard-init'] }), queryClient.invalidateQueries({ queryKey: ['settings-dashboard-projects'] }), queryClient.invalidateQueries({ queryKey: ['report'] }), queryClient.invalidateQueries({ queryKey: ['cycles-folders'] })]);
    },
  });
  ...
  const syncResult = syncLiveMutation.data as TestResult | undefined;
  ...
  function saveAll() { setUserLlmSelection(currentLlmSelection()); setReportBranding(reportBranding); setJiraConnections(jiraConnections.map((c) => ({ ...c, deploymentType: c.deploymentType || 'on-prem' }))); setQmetryConnections(qmetryConnections.map((c) => ({ ...c, cycleIds: [] }))); setSavedMsg('Saved. Use Settings → Sync JIRA/QMetry to refresh API data, or Import Data → Sync imported data for files only.'); queryClient.invalidateQueries({ queryKey: ['integrations'] }); queryClient.invalidateQueries({ queryKey: ['cycles-folders'] }); queryClient.invalidateQueries({ queryKey: ['dashboard-init'] }); setTimeout(() => setSavedMsg(null), 3500); }
